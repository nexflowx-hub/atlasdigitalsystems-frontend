...
- Currencies presented by the catalogue: USD (primary), GBP, EUR, BRL
- Atlas is the direct seller of its own products and services. Online checkout uses the payment provider enabled for the actual transaction; Stripe Checkout is the primary planned integration.
...
- Never state that checkout or a payment method is live unless the website actually presents it as available.
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (features.aiProvider !== "zai") {
      const mockReply = `Thanks for your message — I'm the Atlas Assistant demo. Live AI support is not enabled in this environment. Please email ${company.emails.support} or call ${company.phone.display} and our team will help you.`;
      return NextResponse.json({ reply: mockReply, mock: true });
    }
    ...
    if (!reply) return NextResponse.json({ error: "No response generated." }, { status: 502 });
